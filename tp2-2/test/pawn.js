...
describe("Black pawn", function () {
  describe("#canMove() from its initial position", function () {
    ...
    it("should not be able to move up", function () {
      const output = pawn.canMove(pawn.rank + 1, pawn.file),
        oracle = false;

      assert.equal(output, oracle);
    });
    it("should not be able to move up 2 ranks from a non-initial position", function () {
      const output = pawn.canMove(pawn.rank - 3, pawn.file),
        oracle = false;

      assert.equal(output, oracle);
    });
    it("Should be in right position", function () {
      assert.equal(pawn.isInInitialPosition(), true);
    });
  });
});
