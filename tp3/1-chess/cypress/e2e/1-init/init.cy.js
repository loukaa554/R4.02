...
describe("Init the chessboard correctly", () => {
  ...
  // Check the piece special
  for (let i = 1; i <= 8; i++) {
    if (i === 1 || i === 8) {
      it(`should contain a white ${game[i - 1].type} at ${convertFileToString(
        i
      )}1`, () => {
        cy.get(`#chessboard .rank:last-child div:nth-child(${i}) img`)
          .should("exist")
          .invoke("attr", "src")
          .should("eq", `assets/img/icons/_white.png`);
      });
    }
  }
});
